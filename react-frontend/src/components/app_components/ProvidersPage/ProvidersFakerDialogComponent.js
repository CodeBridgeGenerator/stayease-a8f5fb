...
import providersFakerFactory from "./providersFakerFactory";
...
const ProvidersFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = providersFakerFactory(props.user,fakerCount,providerIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("providers").create(elem));
...
export default connect(mapState, mapDispatch)(ProvidersFakerDialogComponent);
