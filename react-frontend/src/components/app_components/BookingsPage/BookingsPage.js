...
const BookingsPage = (props) => {
    ...
    useEffect(() => {
        ...
        client
            .service("bookings")
            .find({ query: { $limit: 10000 , listingId : urlParams.singleListingsId ,customerId : urlParams.singleUsersId ,providerId : urlParams.singleUsersId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="bookings-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-11" role="bookings-datatable">
                    <BookingsDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
