...
import bookingsFakerFactory from "./bookingsFakerFactory";
...
const BookingsFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [listingIdIds, setlistingIdIds] = useState();
...
const [providerIdIds, setproviderIdIds] = useState();
    ...
    useEffect(() => {
        ...
        client.service("listings")
                        .find({ query: { $select: ["_id","name"] } })
                        .then((data) => {
                        setlistingIdIds(data.data);
                        });
...
client.service("users")
    ...
    const onRun = async () => {
        let fakeData = bookingsFakerFactory(props.user,fakerCount,listingIdIds,customerIdIds,providerIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("bookings").create(elem));
...
export default connect(mapState, mapDispatch)(BookingsFakerDialogComponent);
