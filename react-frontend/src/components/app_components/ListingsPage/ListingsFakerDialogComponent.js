...
import listingsFakerFactory from "./listingsFakerFactory";
...
const ListingsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = listingsFakerFactory(props.user,fakerCount,providerIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("listings").create(elem));
...
export default connect(mapState, mapDispatch)(ListingsFakerDialogComponent);
