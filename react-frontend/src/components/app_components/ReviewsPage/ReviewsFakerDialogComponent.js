...
import reviewsFakerFactory from "./reviewsFakerFactory";
...
const ReviewsFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [bookingIdIds, setbookingIdIds] = useState();
const [customerIdIds, setcustomerIdIds] = useState();
const [providerIdIds, setproviderIdIds] = useState();
    ...
    useEffect(() => {
        ...
        client.service("bookings")
                        .find({ query: { $select: ["_id","bookingDate"] } })
                        .then((data) => {
                        setbookingIdIds(data.data);
                        });
client.service("users")
                        .find({ query: { $select: ["_id","name"] } })
                        .then((data) => {
                        setcustomerIdIds(data.data);
                        });
client.service("users")
                        .find({ query: { $select: ["_id","name"] } })
                        .then((data) => {
                        setproviderIdIds(data.data);
                        });
    ...
    const onRun = async () => {
        let fakeData = reviewsFakerFactory(props.user,fakerCount,bookingIdIds,customerIdIds,providerIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("reviews").create(elem));
...
export default connect(mapState, mapDispatch)(ReviewsFakerDialogComponent);
