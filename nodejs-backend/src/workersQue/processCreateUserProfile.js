...
const createUserProfile = (app) => {
    ...
    const worker = new Worker(
        ...
        async (job) => {
            ...
            // app.service('profiles').create(_profile); // Removed to prevent duplicate profiles
        },
        { connection }
    );
    ...
    worker.on('failed', async (job, err) => {
        ...
        if (job.data) {
            const _mail = {
                ...
                recipients: ['s.khadijahalias@gmail.com'],
                status: false,
                data: {...job.data},
    ...
    userService.hooks({
        after: {
            create: async (context) => {
                // await jobQueue.add('createUserProfile', result); // Remove if not needed
                return context;
            }
        }
    });
